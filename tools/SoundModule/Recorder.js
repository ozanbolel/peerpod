...
export class Recorder {
  ...
  constructor(context, bufferSize, numberOfInputs, numberOfOutputs) {
    ...
    this.trackLs = [];
    /** @type {Array.<Array.<Float32Array>>} 2 dimensions array */
    this.trackRs = [];
  ...
  clear(track) {
    ...
    if (String(track).toLowerCase() === "all") {
      for (const track of this.trackLs) {
        track.length = 0;
      }
      for (const track of this.trackRs) {
        track.length = 0;
      }
    } else {
      if (this.isTrack(track)) {
    ...
    switch (QBIT) {
      case 8:
        ...
        for (let i = 0; i < SIZE; i++) {
          ...
          if (binary > Math.pow(2, 8) - 1) {
            binary = Math.pow(2, 8) - 1;
          }
          if (binary < Math.pow(2, 0) - 1) {
            binary = Math.pow(2, 0) - 1;
          }
      ...
      case 16:
        ...
        for (let i = 0; i < SIZE; i++) {
          ...
          // for preventing from clipping
          if (binary > +Math.pow(2, 15) - 1) {
            binary = Math.pow(2, 15) - 1;
          }
          if (binary < -Math.pow(2, 15) - 1) {
            binary = -Math.pow(2, 15) - 1;
          }
